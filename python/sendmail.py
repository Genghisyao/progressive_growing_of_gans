    ...
    def controller(self):
        ...
        if statusResult:
            for q in sendResult:
                ...
                # 警戒线预警
                if warning_type == 'line':
                    resultList = self.lineMessage(condition)
                    # for r in resultList:
                    #     print r[0],r[1],r[2],r[3],r[4],r[5],r[6]
                    content = self.htmlContent(resultList)
                    for m in method:
                        if m['type'] == 'email':
                            to_addr = ','.join(m['value'])
                            # print to_addr
                            self.sendMail(to_addr, u"警戒线预警通知", content, html=True)
                            print u'邮件已发送'
            ...
            <table style="border-collapse:collapse;">
                <thead>
                    <tr>
                        ...
                        <th style="font-weight: 700;border:1px solid #000;padding:5px 10px;">事件</th>
